...
export function Leaderboard({ rows }: LeaderboardProps) {
  ...
  return (
    <div className="rounded-xl border border-line bg-surface px-5 py-[18px]">
      ...
      <div
        className={cn(
          "grid items-center gap-3 border-b border-line pb-2.5 text-meta text-fg-tertiary",
          GRID_COLS,
        )}
      >
        <span>Agent</span>
      ...
      {rows.map((row, i) => (
        <div
          key={row.agentId}
          className={cn(
            "grid items-center gap-3 py-3 text-ui",
            GRID_COLS,
        ...
        >
          <span className="flex min-w-0 items-center gap-2.5">
            <span
              className="grid h-5 w-5 shrink-0 place-items-center rounded-[6px] text-micro font-semibold text-white"
              style={{
