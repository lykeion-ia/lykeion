 ...
 * runtime's own kernel host, made through `RunRelay.deliverNow` and
...
/** How long one runtime is given to answer before it is treated as holding
 ...
 *  fans this out to every runtime in the lab at once and a researcher
 ...
 *  field that call reports, none of which name a runtime or a Study: the
...
export interface KernelListRegistry {
  ...
  await(runtimeId: string, requestId: string, timeoutMs?: number): Promise<RawKernelReport[]>;
  /**
   * What a machine claiming to be `runtimeId` reported for `requestId`.
   ...
   * `runtimeId` or a `requestId` nobody is waiting on — the two are
   ...
   * never consumes the entry: the runtime this request actually belongs to
   * can still settle it correctly afterward.
   */
  settle(runtimeId: string, requestId: string, kernels: RawKernelReport[]): boolean;
}

export function createKernelListRegistry(): KernelListRegistry {
  const waiting = new Map<string, { runtimeId: string; resolve: (kernels: RawKernelReport[]) => void }>();
  return {
    ...
    settle(runtimeId, requestId, kernels) {
