...
// `listAgents`, `upsertAgent` or `listConnectorTools` at
...
function depsFor(store: Store, now: () => number = () => NOW): Deps {
  ...
  return {
    ...
    coordinator: createEnvironmentSetupCoordinator({ store, runs, now }),
...
it("keeps a Group's absent lead reading back as an absent key, not null", async () => {
  const store = freshStore();
  addOwner(store, "u_owner");
  const config = configSurfaceApi(depsFor(store));

  const bare = await config.createGroup({ name: "No lead yet" });
  expect("leadAgent" in bare).toBe(false);
  expect(bare.memberAgents).toEqual([]);

  const led = await config.createGroup({ name: "Led", leadAgent: "atlas" });
  expect(led.leadAgent).toBe("atlas");
});

it("lists Groups newest first, insertion order breaking a same-second tie", async () => {
  ...
  const first = await config.createGroup({ name: "First" });
  const second = await config.createGroup({ name: "Second" });
  expect((await config.listGroups()).map((g) => g.id)).toEqual([second.id, first.id]);
});
